<?php 
	...
	if (isset($_POST['add_to_cart'])) {
		...
		$select_cart = mysqli_query($conn, "SELECT * FROM `cart` WHERE menu_name='$name'");
		if (mysqli_num_rows($select_cart)>0) {
			$message[] = 'product already added in your cart';
		}else{
			$query = "INSERT INTO `cart`(`menu_name`, `menu_price`,`menu_image`,`menu_quantity`) VALUES ('$name', '$price', '$image', '$quantity')";
